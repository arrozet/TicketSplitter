...
@pytest.mark.xfail(reason="No hay validaciones estrictas en el modelo Item")
def test_item_creation_invalid():
    """Prueba la creación de un Item con datos inválidos"""
    # Arrange
    invalid_data = {
        "id": 1,
        "name": "",  # Nombre vacío
        "quantity": -1,  # Cantidad negativa
        "price": -2.50,  # Precio negativo
        "total_price": 2.50
    }
...
@pytest.mark.xfail(reason="No hay validaciones estrictas en ReceiptParseResponse para totales")
def test_receipt_parse_response_creation_invalid_totals():
    """Prueba que no se puede crear un ReceiptParseResponse con totales inconsistentes"""
    # Arrange
    items = [
        Item(id=0, name="Café", quantity=1, price=2.50, total_price=2.50),
        Item(id=1, name="Tostada", quantity=2, price=3.00, total_price=6.00)
    ...
    with pytest.raises(ValidationError) as exc_info:
        ReceiptParseResponse(
            receipt_id="test-123",
            items=items,
            subtotal=10.00,  # Subtotal incorrecto
            tax=0.85,
            total=9.35,
            upload_timestamp=timestamp,
            is_ticket=True
...
@pytest.mark.xfail(reason="No hay validaciones estrictas en ReceiptParseResponse para lista vacía de items")
def test_receipt_parse_response_creation_empty_items():
    """Prueba que no se puede crear un ReceiptParseResponse sin items"""
    # Arrange
    timestamp = datetime.now().isoformat()
