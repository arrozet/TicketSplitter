...
def test_init_with_api_key_sets_up_model(mock_genai):
    """Prueba que la inicialización con API key configura correctamente el modelo."""
    from app.services.ocr_service import OCRService
    mock_model = MagicMock()
    mock_genai.configure = MagicMock()
    mock_genai.GenerativeModel.return_value = mock_model
    ocrService = OCRService(api_key="test_api_key")
    mock_genai.configure.assert_called_once_with(api_key="test_api_key")
    mock_genai.GenerativeModel.assert_called_once()
    assert ocrService.model is not None

@patch('google.generativeai')
def test_init_without_api_key_raises_error(mock_genai):
...
@patch('google.generativeai')
def test_extractTextFromImage_valid_ticket_returns_json(mock_genai, sample_image_bytes):
    ...
    mock_response.parts[0].text = json.dumps(expected_json)
    mock_model.generate_content.return_value = mock_response
    
    resultado = ocrService.extractTextFromImage(sample_image_bytes)
...
@patch('google.generativeai')
def test_extractTextFromImage_not_ticket_returns_error_json(mock_genai, sample_image_bytes):
...
@patch('google.generativeai')
def test_extractTextFromImage_empty_response_returns_empty_string(mock_genai, sample_image_bytes):
...
@patch('google.generativeai')
def test_preprocessImageForOcr_valid_bytes_returns_image(mock_genai, sample_image_bytes):
    ...
    resultado = ocrService._preprocessImageForOcr(sample_image_bytes)
    assert isinstance(resultado, Image.Image)
    assert resultado.mode == 'RGB'

@patch('google.generativeai')
def test_preprocessImageForOcr_invalid_bytes_raises_error(mock_genai):
    ...
    with pytest.raises(ValueError) as exc_info:
        ocrService._preprocessImageForOcr(invalid_bytes)
    assert "Los bytes de la imagen no son válidos" in str(exc_info.value)

@patch('google.generativeai')
def test_extractTextFromImage_handles_markdown_json(mock_genai, sample_image_bytes):
...
@patch('google.generativeai')
def test_extractTextFromImage_invalid_json_raises_error(mock_genai, sample_image_bytes):
    ...
    mock_response = MagicMock()
    mock_response.parts = [MagicMock()]
    mock_response.parts[0].text = "invalid json"
    mock_model.generate_content.return_value = mock_response
    
    with pytest.raises(RuntimeError) as exc_info:
        ocrService.extractTextFromImage(sample_image_bytes)
